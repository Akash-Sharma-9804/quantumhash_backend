...
exports.uploadFiles = async (req, res) => {
  try {
      ...
      for (const file of files) {
          ...
          try {
              ftpPath = await uploadToFTP(buffer, fileName);
              // Store the file path and file name for later use
              if (ftpPath) {
                  filePaths.push(ftpPath);
      ...
      // Create the user message with file names
      let fullUserMessage = userMessage || "Uploaded files";
